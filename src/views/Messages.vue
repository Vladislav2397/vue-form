...
export default class Messages extends Vue {
    listCompleteAnimationName: "list-complete-top" | "list-complete-bottom" =
        "list-complete-top";
    ...
    messages: Record<
        string,
        { id: number; text: string; status: "s" | "w" | "e" }
    > = {
        1: {
            id: 1,
            text: "some text",
    ...
    fetch() {
        ...
        this.$set(this.group, "01.04", {
            date: "01.04",
            messages: [...(this.group?.["01.04"]?.messages ?? []), firstId],
        })

        this.$set(this.group, "31.03", {
            date: "31.03",
            messages: [...(this.group?.["31.03"]?.messages ?? []), secondId],
        })
...
.list-complete-top-enter,
.list-complete-top-leave-to {
    opacity: 0;
...
.list-complete-bottom-enter,
.list-complete-bottom-leave-to {
    opacity: 0;
    transform: translateY(-30px);
}
